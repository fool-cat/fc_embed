...
static int __fc_port_vprintf_write(FC_FILE* f, const void* buf, int len)
{
    if (len > 0)
    {
        fc_fifo_linear_write_done((fc_port_t*)(f->user), len);

        // 还有空间
        if (fc_fifo_get_free(((fc_port_t*)(f->user))->rb) >= 1)
        {
            f->p_start = (char*)fc_fifo_linear_write_setup(((fc_port_t*)(f->user))->rb, &size);
    ...
    {
        // 为0表示结束,解算一下写入的字节数,同时将环形缓冲区的写指针移动到最新位置
        f->n += (size_t)buf - (size_t)f->p_start;
        fc_fifo_linear_write_done((fc_port_t*)(f->user), (size_t)buf - (size_t)f->p_start);
    }
