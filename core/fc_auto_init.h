...
/** fc_auto_init component version string — keep in sync with Cversion in fool_cat.fc_embed.pdsc */
#define FC_AUTO_INIT_VERSION "2.0.0"
...
#define FC_STRINGFY(x) __FC_STRINGFY(x)
    ...
    #define SECTION_EXTERN(section_name)                        \
        extern const char FC_CONNECT(__start_, section_name)[]; \
        extern const char FC_CONNECT(__stop_, section_name)[]
